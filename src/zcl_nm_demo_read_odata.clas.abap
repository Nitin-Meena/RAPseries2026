CLASS zcl_nm_demo_read_odata DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_NM_DEMO_READ_ODATA IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    DATA:
       lt_business_data TYPE TABLE OF znmcl_rap_companynames=>tys_company_names_type.

    TRY.
        DATA(lo_client_proxy) = cl_web_odata_client_factory=>create_v2_remote_proxy(
          EXPORTING
            iv_service_definition_name = 'ZNM_DEMO_RAP_ONPREM_ODATA'
            io_http_client             = get_client( )
            iv_relative_service_root   = '/sap/opu/odata/sap/ZNM_API_COMPANY_NAMES_O2' ).
